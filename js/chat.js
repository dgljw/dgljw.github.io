...
const Chat = {
    ...
    async checkFeatureStatus() {
        this.setFeatureStatus('memory', true);
        this.setFeatureStatus('image', true);
        
        await this.checkSearchStatus();
        await this.checkEmojiStatus();
    },
    ...
    async checkSearchStatus() {
        try {
            ...
            const data = await res.json();
            if (data.content) {
                this.setFeatureStatus('search', true);
            } else {
                this.setFeatureStatus('search', false);
            }
        } catch {
            this.setFeatureStatus('search', false);
        }
    ...
    /** 检查表情包功能状态 */
    async checkEmojiStatus() {
        try {
            const res = await fetch(`${CONFIG.EMOJI_API}?keyword=test&count=1`, {
                signal: AbortSignal.timeout(5000)
            });
            const data = await res.json();
            if (data.images && data.images.length > 0) {
                this.setFeatureStatus('emoji', true);
            } else {
                this.setFeatureStatus('emoji', false);
            }
        } catch {
            this.setFeatureStatus('emoji', false);
        }
    ...
    /** 设置功能状态指示器 */
    setFeatureStatus(feature, online) {
        const el = document.getElementById(`status-${feature}`);
        if (el) {
            el.className = `feature-status ${online ? 'online' : 'offline'}`;
        }
    },
    ...
    async callChatAPI(userContent) {
        ...
        const messages = [timeContext, context, ...recentMessages, { role: 'user', content: userContent }];
